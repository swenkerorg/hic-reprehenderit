# @swenkerorg/hic-reprehenderit

![CI](https://github.com/swenkerorg/hic-reprehenderit/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@swenkerorg/hic-reprehenderit.svg?style=flat)](https://www.npmjs.com/package/@swenkerorg/hic-reprehenderit)
...
```shell
npm i @swenkerorg/hic-reprehenderit
# or 
yarn add @swenkerorg/hic-reprehenderit
```
...
fastify.register(require('@swenkerorg/hic-reprehenderit'))
...
Routes registered with `@swenkerorg/hic-reprehenderit` respect the Fastify plugin encapsulation contexts, and so will run any hooks that have been registered. This means the same route hooks you might use for authentication or error handling of plain old HTTP handlers will apply to websocket handlers as well.
...
- When using `@swenkerorg/hic-reprehenderit`, it needs to be registered before all routes in order to be able to intercept websocket connections to existing routes and close the connection on non-websocket routes.

```js
import Fastify from 'fastify'
import websocket from '@swenkerorg/hic-reprehenderit'
...
```js
const Fastify = require('fastify')
const FastifyWebSocket = require('@swenkerorg/hic-reprehenderit')
const ws = require('ws')
...
`@swenkerorg/hic-reprehenderit` accept these options for [`ws`](https://github.com/websockets/ws/blob/master/doc/ws.md#new-websocketserveroptions-callback) :
...
_**NB** By default if you do not provide a `server` option `@swenkerorg/hic-reprehenderit` will bind your websocket server instance to the scoped `fastify` instance._

_**NB** The `path` option from `ws` should not be provided since the routing is handled by fastify itself_

_**NB** The `noServer` option from `ws` should not be provided since the point of @swenkerorg/hic-reprehenderit is to listen on the fastify server. If you want a custom server, you can use the `server` option, and if you want more control, you can use the `ws` library directly_
